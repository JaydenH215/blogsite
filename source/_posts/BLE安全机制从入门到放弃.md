...
上一节，我们分析了ble4.0的安全“漏洞"，如果Eve一直监听Alice和Bob的配对阶段，加密会形同虚设。所以ble4.2有新的解决方案，我们先分析一下有哪些无法改变的前提，以及需要解决的问题，再来说说ble4.2的做法。
...
1. Eve窃听整个过程，从而破解TK，从而得到STK，从而得到LTK，使得Bob和Alice通讯之间没有机密性；
2. Mallory伪装成Bob和Alice，实现中间人攻击。

提出解决的方案：

1. 因为配对码范围很窄，认证码终究会被穷举破解，所以要用动态认证码；
2. 因为配对码范围很窄，认证码终究会被穷举破解，所以加密不能依赖认证码，用ECDH可保证机密性；
